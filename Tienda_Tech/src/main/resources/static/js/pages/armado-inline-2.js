(() => {
        // ====== Config ======
        const CAT_TO_ID = {
            storage:1, cpu:2, cooling:3, case:4, psu:5, gpu:6, ram:7, mobo:8, periferico:9
        };
        const SLOTABLE = new Set(Object.keys(CAT_TO_ID).filter(k => k !== 'periferico'));
        const SAVE_KEY = 'pcbuilder.selection';

        const API = {
            byCatV1:  id => `/api/productos/por-categoria?categoriaId=${encodeURIComponent(id)}`,  // nuevo
            byCatV0:  id => `/api/busqueda/categoria/${id}`,                                       // antiguo
        ...
        function showSnack(){ ttSetStyle(snack, 'opacity', '1'); setTimeout(()=> ttSetStyle(snack, 'opacity', '0'), 1600); }

        // ====== Estado & helpers ======
        const topImgs = {}; // img de cada categoría en la franja superior
        catItems.forEach(el => { topImgs[el.dataset.cat] = el.querySelector('img'); });
        ...
        async function fetchByCat(catId){
            // intenta endpoint nuevo y luego el antiguo
            try {
        ...
        // (Opcional) cargar una por defecto
        // loadCat('cpu');
    })();
